...
# 43 "Esclavo1.c"
uint8_t TEMPORAL;
unsigned short POT_3;
unsigned short new_POT_3;
unsigned short POT_4;
unsigned short new_POT_4;

float val_ccp1 = 50.0f;
...
void __attribute__((picinterrupt(("")))) isr (void){
    if (PIR1bits.SSPIF){
        ...
        if(PORTBbits.RB7 == 0){
            POT_3 = map(TEMPORAL, 0, 255, 135, 590);
        }
        else if(PORTBbits.RB7 == 1){
            POT_4 = map(TEMPORAL, 0, 255, 135, 590);
        }
        PIR1bits.SSPIF = 0;
    }
    return;
...
void main(void) {
    ...
    while(1){
        ...
        CCPR1L = (uint8_t)(new_POT_3>>2);
        CCP1CONbits.DC1B = new_POT_3 & 0b11;

        CCPR2L = (uint8_t)(new_POT_4>>2);
        CCP2CONbits.DC2B0 = new_POT_4 & 0b01;
        CCP2CONbits.DC2B1 = ((new_POT_4 & 0b10)>>1);
    }
    return;
}
